...
const getAllUsers = async (req, res, next) => {
  try {
    ...
    const users = await User.find(query).select('-password').sort(sort).skip(skip).limit(parseInt(limit));
  ...
  try {
    ...
    if (sendEmail) {
      try {
        ...
        logger.error(`Failed to send account locked email: ${emailError.message}`, {
          error: emailError
        });
      }
    }
...
const getUserSessions = async (req, res, next) => {
  try {
    ...
    const sessions = await Session.find({
      userId
    }).select('-token -refreshToken').sort({
      lastActivity: -1
    });
    res.status(200).json({
      success: true,
...
const getUserSecurityLogs = async (req, res, next) => {
  try {
    ...
    const logs = await SecurityLog.find(query).sort({
      createdAt: -1
