...
const { password: passwordConfig } = require('../config').security;

// Password validation pattern based on configuration
const passwordPattern = new RegExp(
  `^${passwordConfig.requireUppercase ? '(?=.*[A-Z])' : ''}` +
  `${passwordConfig.requireLowercase ? '(?=.*[a-z])' : ''}` +
  `${passwordConfig.requireNumbers ? '(?=.*\\d)' : ''}` +
  `${passwordConfig.requireSymbols ? '(?=.*[!@#$%^&*(),.?":{}|<>])' : ''}` +
...
const registerSchema = Joi.object({
  ...
  role: Joi.string()
    .valid('ADMIN', 'MANAGER', 'STAFF', 'DRIVER', 'CUSTOMER')
    .default('CUSTOMER'),
});
...
const adminCreateUserSchema = Joi.object({
  ...
  phoneNumber: phoneNumber.optional(),
  role: Joi.string()
    .valid('user', 'admin', 'manager', 'driver', 'warehouse')
    .default('user'),
...
const adminLockAccountSchema = Joi.object({
  reason: Joi.string()
    .valid('MANUAL', 'SUSPICIOUS_ACTIVITY', 'PASSWORD_EXPIRED')
    .default('MANUAL'),
  sendEmail: Joi.boolean().default(true),
});
